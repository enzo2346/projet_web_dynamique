...
<html>
  <head>
    <title>Omnes Santé Rendez-Vous</title>
    <meta charset="utf-8" />
  ...
  <thead>
    <tr>
      ...
      <th scope="col">Spécialité ou Service</th>
  ...
  <tbody>
    ...
    //si la BDD existe
    if ($db_found) {
        $sql = "SELECT rendezvous.idpersonnel,rendezvous.date,rendezvous.creneau,personnel.nom,personnel.prenom,personnel.specialite,personnel.salle FROM rendezvous, personnel WHERE idclient='$id' AND rendezvous.idpersonnel = personnel.idpersonnel;";
       ...
       <tr>
        ...
        <td><form action="annulerdv.php" method="post"><input type="hidden" name="idpersonnel" value='.$idpersonnel.'><input type="hidden"  name="idclient" value='.$id.'><input type="hidden" name="date" value='.$date.'><input type="hidden" name="creneau" value='.$heure.'><button type="submit" class="btn btn-danger" data-mdb-ripple-color="dark" name="Valider">Annuler</button></form></td>
      </tr>';
            }
        }
    }
